...
- If the resource file does NOT exist (e.g. spec has `/v1/newsletters` but no resource file), **CREATE the file** following the structure below + wire it into the SDK entrypoint (src/resources/index.ts + the `Blueticks` class constructor in src/client.ts).
